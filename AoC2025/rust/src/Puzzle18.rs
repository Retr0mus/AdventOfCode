...
const SIZE: usize = 100_000;   // 98_416
...
fn inside(figure : &Vec<Vec<i8>>, point : (usize, usize)) -> bool {
    figure[point.0][point.1] <= 1
}


fn line_inside(figure : &Vec<Vec<i8>>, line : &Vec<(i32,i32)>) -> bool {
    let result : bool = line.into_iter().map(|(x,y)| inside(figure, (*x as usize, *y as usize))).fold(true, |x, acc| x && acc);
    result
}
...
fn partial_flood_fill(matrix: &mut Vec<Vec<i8>>, initial_x: usize, initial_y: usize, target_colors: Vec<i8>, replacement_color: i8, start : bool) {
...
fn main() -> io::Result<()> {
    ...
    println!("Result: {}", area);    // 1343576598
